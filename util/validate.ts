...
const filename = ow.optional.string.is((s) =>
  (s.indexOf("/") === -1 && s.indexOf("\\") === -1) ||
  `Filename must not include slashes, got \`${s}\``
);
const filenameReq = ow.string.is((s) =>
  (s.indexOf("/") === -1 && s.indexOf("\\") === -1) ||
  `Filename must not include slashes, got \`${s}\``
);

export const chapterPredicate: ObjectPredicate<Chapter> = ow.object
  .partialShape({
...
export const optionsPredicate: ObjectPredicate<Options> = ow.object
  .partialShape({
    title: ow.string,
    author: name,
    ...
    version: ow.optional.number.is((x) =>
      x === 3 || x === 2 ||
